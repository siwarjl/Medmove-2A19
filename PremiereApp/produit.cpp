...
{
    // Prompt the user to select a file name and location to save the PDF file

    QString fileName = QFileDialog::getSaveFileName((QWidget* )0, "Export PDF", QString(), "*.pdf");
    // Check if the file name has an extension, and if not, add the ".pdf" extension

    if (QFileInfo(fileName).suffix().isEmpty()) { fileName.append(".pdf"); }
    // Create a QPrinter object to print to a PDF file
...
void produit::on_pushButton_22_clicked()
{
        ...
        // ui->listView->setModel(p.afficher());
        QMessageBox::warning(this, "traductiion", "Il est deja en français !");
...
void produit::on_view_pressed(const QModelIndex &index)
{
    using namespace qrcodegen;
         ...
         QString marque = model->data(model->index(row, 2)).toString();
         ...
         // Créer le QR code
            QString text;
            text.push_back("les cordonnes de produit : \n");
            text.push_back(" ID: " + id + "\n" + "NOM: " + nom + "\n" + "marque: " + marque + "\n" );


            // Encode the text as a QR code with medium error correction level
            QrCode qr = QrCode::encodeText(text.toUtf8().data(), QrCode::Ecc::MEDIUM);

            // Get the size of the QR code and create a new QImage object of the same size
            qint32 sz = qr.getSize();
            QImage im(sz, sz, QImage::Format_RGB32);

            // Define the black and white colors to use for the QR code image
            QRgb black = qRgb(0, 0, 0);
            QRgb white = qRgb(255, 255, 255);

            // Iterate over each module in the QR code and set the corresponding pixel in the image to black or white
            for (int y = 0; y < sz; y++) {
                for (int x = 0; x < sz; x++) {
            ...
            // Scale the image to 100x100 pixels while maintaining its aspect ratio, and display it in a QLabel
            ui->label1->setPixmap(QPixmap::fromImage(im.scaled(100, 100, Qt::KeepAspectRatio, Qt::FastTransformation), Qt::MonoOnly));

}
void produit::showTime(){
...
{
//data2 = A.read_from_arduino();

        data = A.read_from_arduino();
        //int bpm = data.toInt();
        //coeur_2->setText(QString::number(bpm));

               ui->coeur_2->setText(data); // Afficher la chaîne de caractères dans l'interface utilisateur
 //ui->coeur_22->setText(data2);
             //  ui->coeur_2->setText(QString::number(bpm)); // Afficher la chaîne de caractères dans l'interface utilisateur

                  QString coeur = ui->coeur_2->text();


                 patientarduinoc p(coeur);
                 ...
                 ui->tview->setModel(p.afficher());

                      }
}

void produit::on_pushButton_16_clicked()
{
    A.write_to_arduino("0");
    QMessageBox::information(nullptr,"arduino","capteur désactiver");
    ui->coeur_2->clear();
...
void produit::on_pushButton_21_clicked()
{
    A.write_to_arduino("1");
  //  update_label();
          //  QThread::msleep(50);

 QMessageBox::information(nullptr,"arduino","capteur activer");

}

/*void produit::on_pushButton_24_clicked()
{
    QString nom = ui->nom_2->text();
       QString datee = ui->datee->text();
       QString coeur = ui->coeur_2->text();
